...
mod tests {
    use super::*;
    use l2r_core::model::generic_number::GenericNumRange;

    #[test]
    fn test_id_range_display() {
        let start = Id::from(540);
        let range = GenericNumRange::from_id(start, 100);

        assert_eq!(format!("{}", range), "00500-00599");
    }
}
