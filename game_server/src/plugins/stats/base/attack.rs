...
pub struct AttackStatsPlugin;
impl Plugin for AttackStatsPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(AttackStatsComponentsPlugin);
        app.add_systems(
            Update,
            on_required_components_changed.in_set(StatKindSystems::Attack),
        );
    }
}

fn on_required_components_changed(mut args: StatsCalcParams<AttackStats>) -> Result<()> {
